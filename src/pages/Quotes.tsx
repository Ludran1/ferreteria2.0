...
import { useClients, useClientPayments } from '@/hooks/useClients';
...
export default function Quotes() {
  ...
  const [transactionType, setTransactionType] = useState<'quote' | 'sale'>('sale');
  ...
  const { addPayment } = useClientPayments(); // Generic hook usage
  ...
  const handleSaveAndPrint = async () => {
    ...
    try {
        ...
        if (transactionType === 'sale') {
           ...
           if (paymentType === 'credito') {
               if (!selectedClient) {
                   toast({
                       ...
                       description: "Para ventas a crédito, debes seleccionar un cliente registrado.",
                       variant: "destructive"
                   });
                   return;
               }
           ...
           // Create Sale
           result = await createSale.mutateAsync({
               ...transactionData,
               paymentMethod,
               paymentType,
               clientId: selectedClient?.id,
               clientId: selectedClient?.id,
               balance: paymentType === 'credito' ? total : 0 // Balance starts at Total, reduced by addPayment later
           });
           
           // Handle Advance Payment if Credit
           if (paymentType === 'credito' && advanceAmount && parseFloat(advanceAmount) > 0) {
               await addPayment.mutateAsync({
                   clientId: selectedClient!.id,
                   saleId: result.id,
                   amount: parseFloat(advanceAmount),
                   paymentMethod: paymentMethod, // Using the same method selected for the sale
                   date: new Date(),
                   notes: 'Adelanto inicial (Venta)'
               });
           }

        } else {
           result = await createQuote.mutateAsync({
               ...transactionData,
               status: 'pending',
           });
        }
  ...
  const getPrintData = (): PrintableDocumentData => {
    const docNumber = lastSavedTransaction 
        ? (transactionType === 'sale' ? lastSavedTransaction.invoice_number : lastSavedTransaction.quote_number)
        : 'PENDIENTE';
    ...
    return {
        ...
        paymentType: transactionType === 'sale' ? paymentType : undefined,
    };
  };
        ...
        <div className="hidden lg:block lg:col-span-3">
          <div className="sticky top-24 rounded-2xl bg-card p-6 shadow-lg">
            ...
            <div className="mt-6 space-y-3">
              ...
              {/* Removed redundant manual inputs block that was here */}

              
              {/* Advance Payment for Credit Sales */}
              {transactionType === 'sale' && paymentType === 'credito' && (
                  <div className="grid grid-cols-2 gap-3 p-3 bg-secondary/30 rounded-lg border border-primary/20">
                      <div className="flex flex-col justify-center">
                          <Label className="text-primary font-semibold mb-1">Adelanto (Abono Inicial)</Label>
                          <p className="text-xs text-muted-foreground">Opcional. Se registrará como pago.</p>
                      </div>
                      <div className="relative">
                          <span className="absolute left-3 top-2.5 text-muted-foreground">S/</span>
                          <Input 
